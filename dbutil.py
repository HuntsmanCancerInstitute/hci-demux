...
def Dump( conn ):
	c = conn.cursor()
	c.execute("select id, dirname, state from run order by id")
	recs = c.fetchall()
	for rec in recs:
		print TAB.join(list(rec))

def AddRun( conn, run_id, dirname ):
	cursor = conn.cursor()
	t = ( run_id, dirname, 'a_new' )
	try:
		retval1 = cursor.execute("insert into run (id,dirname,state) values ('%s','%s','%s')" % t )
		retval2 = conn.commit()
...
def usage():
	print "Use: %s --help | --state <run_id> <state_name> | --delete <run_id> | --add <run_id> <dirname> | --dump" % sys.argv[0]
